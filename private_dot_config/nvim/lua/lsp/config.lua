...
-- vim.cmd([[
--   autocmd! CursorHold,CursorHoldI * lua vim.diagnostic.open_float({focus=false, border='rounded'})
-- ]])
local handlers = {
...
local servers = { "pyright", "rust_analyzer", "sumneko_lua", "volar", "lemminx", "marksman" }

local callbacks = {
	["sumneko_lua"] = function(client)
...
for _, lsp in pairs(servers) do
	local c = {
		on_attach = function(client, bufnr)
			...
			if callbacks[lsp] then
				callbacks[lsp](client)
			end
